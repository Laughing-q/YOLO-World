...
class HuggingCLIPLanguageBackbone(BaseModule):
    ...
    #     txt_feats = self.model.encode_text(text).to(dtype=torch.float32)
    #     txt_feats = txt_feats / txt_feats.norm(p=2, dim=-1, keepdim=True)
    #     txt_feats = txt_feats.reshape(-1, num_per_batch[0], txt_feats.shape[-1])
    #     return txt_feats
...
class PseudoLanguageBackbone(BaseModule):
    ...
    def __init__(self, text_embed_path: str = "", test_embed_path: str = None, init_cfg: OptMultiConfig = None):
...
class MultiModalYOLOBackbone(BaseModule):
    def __init__(self, image_model: ConfigType, text_model: ConfigType, init_cfg: OptMultiConfig = None) -> None:
        super().__init__(init_cfg)
